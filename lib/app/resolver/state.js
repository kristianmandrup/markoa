    ...
    return extend(base(name, config, key), {
      find: function() {
        ...
        try {
          ...
          if (!state) {
            console.warning('WARNING: page state is empty.\nIf you expected to have page data, please check:\n' + statePath);
            return {};
          } else {
            return state;
          }
        } catch (e) {
          console.warning('WARNING: page data could not be loaded. Please check your page data script file for syntax errors:\n' + statePath);
          return {};
        }
      }
    });
